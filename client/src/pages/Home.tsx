...
export default function Home() {
  ...
  return <main className="min-h-screen overflow-x-hidden bg-[#061018] text-[#eff4f1]"><header className="sticky top-0 z-40 border-b border-white/10 bg-[#061018]/90 backdrop-blur-xl"><div className="mx-auto flex min-h-[88px] max-w-[1500px] items-center justify-between gap-3 px-5 py-2 sm:px-8 lg:px-12"><img src={wordmark} alt="fleshsesh | academy" className="h-[58px] w-[180px] object-cover object-center sm:h-[72px] sm:w-[240px]" /><div className="flex items-center gap-4"><button onClick={openSupport} className="hidden text-[10px] font-bold uppercase tracking-[.14em] text-[#f1dca4] sm:block">Get help</button><button onClick={openMember} className="border border-[#e6c887]/65 px-3 py-2 text-[10px] font-bold uppercase tracking-[.12em] text-[#f1dca4]">{isAuthenticated ? "My learning" : "Sign in"}</button></div></div></header>
    <section className="relative overflow-hidden border-b border-white/10 px-5 py-20 sm:px-8 lg:px-12 lg:py-28"><img src={classroomScene} alt="Quiet classroom for adult learning" className="absolute inset-0 h-full w-full object-cover opacity-35" /><div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(4,13,20,.98),rgba(4,13,20,.8),rgba(4,13,20,.42))]" /><div className="relative mx-auto grid max-w-[1500px] gap-10 lg:grid-cols-[1fr_430px] lg:items-end"><div><p className="text-[10px] font-bold uppercase tracking-[.22em] text-[#e6c887]">Your first step · about five minutes</p><h1 className="mt-7 max-w-4xl font-display text-[clamp(5rem,11vw,10rem)] leading-[.69] tracking-[-.075em]">Start one<br /><em className="text-[#e49aa9]">small mission.</em></h1><p className="mt-8 max-w-xl text-base leading-7 text-[#d0ded9]">One idea about bodies, consent, relationships, or wellbeing. One simple action. One clear next step.</p><button onClick={startMission} className="mt-9 inline-flex items-center gap-3 bg-[#e49aa9] px-6 py-5 text-xs font-bold uppercase tracking-[.16em] text-[#061018]">Start my first mission <ArrowUpRight className="h-4 w-4" /></button><p className="mt-5 text-[10px] leading-5 text-[#b7c8c3]">No account, enrolment, personal history, or progress record is needed to try the first step.</p></div><div className="border border-white/20 bg-[#071720]/85 p-6 backdrop-blur-md"><p className="text-[9px] font-bold uppercase tracking-[.18em] text-[#e6c887]">What happens next</p><div className="mt-7 space-y-5">{[["1", "See one idea.", "A short explanation, not a wall of text."], ["2", "Try one action.", "A general scenario—never your personal history."], ["3", "Choose what is next.", "Continue, pause, or ask for simple help."]].map(([number, title, copy]) => <div key={number} className="flex gap-4"><span className="font-display text-4xl text-[#e6c887]">{number}</span><p className="text-sm leading-5 text-[#c9d7d4]"><strong className="block text-[#eff4f1]">{title}</strong>{copy}</p></div>)}</div></div></div></section>
