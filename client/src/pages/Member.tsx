...
export default function Member() {
  ...
  const updateProgress = trpc.learningProgress.upsert.useMutation({ onSuccess: () => utils.learningProgress.list.invalidate() });
  ...
  if (loading) {
    return <div className="flex min-h-screen items-center justify-center bg-[#0b090b] px-5 text-[#f6eee2]"><div className="w-full max-w-sm border border-[#e4bd78]/25 bg-[#160e13] p-8 text-center"><img src={wordmark} alt="fleshsesh academy" className="mx-auto h-12 w-[190px] object-contain" /><p className="mt-7 text-[10px] font-bold uppercase tracking-[0.2em] text-[#e4bd78]">Checking your private learning record</p><div className="mt-5 h-px overflow-hidden bg-white/10"><div className="h-full w-2/5 bg-[#ef789d]" /></div></div></div>;
  }
