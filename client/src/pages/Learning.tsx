...
export default function Learning() {
  ...
  const chooseCourse = (course: Course) => {
    ...
    const facilityQuery = activeFacility ? `&facility=${encodeURIComponent(activeFacility.id)}` : "";
    window.history.replaceState(null, "", `/learn?course=${encodeURIComponent(course.code)}${facilityQuery}${previewQuery}`);
  };
  ...
  if (selectedCourse && selectedModule) {
    const returnToCatalogue = activeFacility ? `/learn?facility=${encodeURIComponent(activeFacility.id)}${previewQuery}` : `/learn${previewMode ? "?preview=academy" : ""}`;
