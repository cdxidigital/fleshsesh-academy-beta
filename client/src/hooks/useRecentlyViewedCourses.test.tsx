...
function mountProbe(onChange: (codes: string[]) => void, onClearReady?: (clear: () => void) => void) {
  ...
  function Probe() {
    const [codes, , clear] = useRecentlyViewedCourses();
...
describe("useRecentlyViewedCourses browser storage synchronization", () => {
  ...
  it("clears only the recently viewed browser-local codes", () => {
    window.localStorage.setItem(recentlyViewedStorageKey, '["FSH 206"]');
    const states: string[][] = [];
    let clear: () => void = () => {};
    mountProbe((codes) => states.push(codes), (nextClear) => { clear = nextClear; });
    act(() => clear());
    expect(window.localStorage.getItem(recentlyViewedStorageKey)).toBeNull();
